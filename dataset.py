...
class xBDImages(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        # paths = ["datasets/xbd/tier3_bldgs/", "datasets/xbd/tier3_bldgs/", "datasets/xbd/tier3_bldgs/"]
        paths: List[str],
        # disasters = ["nepal-flooding", "joplin-tornado", "lower-puna-volcano"]
        disasters: List[str],
        # merge_classes = True
...
class xBDMiniGraphs(torch_geometric.data.Dataset):
    ...
    def processed_file_names(self) -> List[str]:
        ...
        for zone in self.zones:
            if not ((self.labels[self.labels['zone'] == zone]['class'] == 'un-classified').all() or \
                ...
                # print("dataset.py 124line" + self.processed_dir + "zone:" + zone)
    ...
    def get(self, idx: int):
        # print("dataset.py 189line" + self.processed_dir)
        # print("dataset.py 190line" + self.processed_file_names[idx])
